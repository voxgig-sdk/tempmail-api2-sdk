...
The Tempmail API provides users with disposable email addresses that can be used for temporary registrations or verifications.

Learn more about Tempmail API at [api.tempmail.lol/v2](https://api.tempmail.lol/v2/).
...
$domains = $client->Domain()->list();
print_r(array_map(fn($item) => $item->data_get(), $domains));
```
