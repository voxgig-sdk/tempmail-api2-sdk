...
local InboxEntity = {}
InboxEntity.__index = InboxEntity


function InboxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InboxEntity)
  self._name = "inbox"
  self._client = client
...
function InboxEntity:get_name()
  return self._name
end


function InboxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InboxEntity.new(self._client, opts)
end
...
function InboxEntity:mark_deleted()
  self._deleted = true
end


function InboxEntity:deleted()
  return true == self._deleted
end


function InboxEntity:data_set(args)
  if args ~= nil then
...
function InboxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InboxEntity:match_set(args)
  if args ~= nil then
...
function InboxEntity:match_get()
...
function InboxEntity:stream(action, args, callopts)
...
---@param reqmatch InboxRemoveMatch
---@param ctrl? table
---@return Inbox
---@return string? err
function InboxEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function InboxEntity:_run_op(ctx, post_done)
  local utility = self._utility
