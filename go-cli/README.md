...
boru-driven command-line client **and** interactive REPL for the TempmailApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tempmail-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./tempmail-api2-cli --help

# 3. Provide credentials once, via the environment
export TEMPMAIL_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./tempmail-api2-cli list domain
./tempmail-api2-cli list email

# 5. Override the API base URL for a single call
TEMPMAIL_API2_BASE=https://api.example.com ./tempmail-api2-cli list domain

# 6. No arguments -> interactive REPL
./tempmail-api2-cli
tempmail-api2> list domain
tempmail-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tempmail-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/tempmail-api2-cli list domain
   ```
...
Run with no arguments to open a REPL (prompt `tempmail-api2>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tempmail-api2-cli`. |
...
(`./tempmail-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
