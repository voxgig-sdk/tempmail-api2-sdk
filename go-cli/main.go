// Command TempmailApi2-cli is a CLI + REPL for the TempmailApi2 SDK,
...
const prompt = "tempmail-api2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "domain email inbox"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TEMPMAIL_API2_APIKEY (api key), TEMPMAIL_API2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TEMPMAIL_API2_APIKEY carries the API key and
	// TEMPMAIL_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TEMPMAIL_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TEMPMAIL_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTempmailApi2SDK(opts)
