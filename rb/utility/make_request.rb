...
module TempmailApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TempmailApi2Response.new({})
    ...
    if fetch_err
      ...
      response = TempmailApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TempmailApi2Response.new(fetched)
    else
