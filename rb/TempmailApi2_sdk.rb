...
require_relative 'TempmailApi2_types'
...
class TempmailApi2SDK
  ...
  def initialize(options = {})
    ...
    feature_opts = TempmailApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TempmailApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TempmailApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TempmailApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TempmailApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TempmailApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TempmailApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = TempmailApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TempmailApi2Error.new(
        "graphql_error", "TempmailApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TempmailApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
