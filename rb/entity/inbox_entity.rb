...
class InboxEntity
  ...
  def make
    opts = @_entopts.dup
    InboxEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TempmailApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TempmailApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Inbox.
  #
  # @param reqmatch [InboxLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Inbox.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Inbox, Hash] the loaded Inbox; raises TempmailApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TempmailApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Inbox, Hash] the created Inbox; raises TempmailApi2Error on failure
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Inbox, Hash] the removed Inbox; raises TempmailApi2Error on failure
