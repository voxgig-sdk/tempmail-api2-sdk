...
[MCP](https://modelcontextprotocol.io) server exposing the TempmailApi2 SDK as
two agent tools — `tempmail-api2_list` and `tempmail-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tempmail-api2-mcp)
make build

# 2. Provide credentials via the environment
export TEMPMAIL_API2_APIKEY=sk_live_xxx
...
```jsonc
// tempmail-api2_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tempmail-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tempmail-api2 \
     -- "$PWD"/dist/*/tempmail-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `tempmail-api2_list` and `tempmail-api2_load` tools now appear
   in new sessions. Ask the agent to *"list domain using tempmail-api2"*
   and it calls `tempmail-api2_list` with `{"entity":"domain"}`.
...
```sh
export TEMPMAIL_API2_APIKEY=sk_live_xxx            # API key
export TEMPMAIL_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `tempmail-api2_list` tool
...
### Call the `tempmail-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `tempmail-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tempmail-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMPMAIL_API2_APIKEY` | API key sent with every request. |
| `TEMPMAIL_API2_BASE` | Optional override of the API base URL. |
