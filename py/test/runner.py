...
class TempmailApi2TestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TempmailApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TempmailApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TempmailApi2TestRunner.getenv("TEMPMAIL_API2_TEST_LIVE")
        override = TempmailApi2TestRunner.getenv("TEMPMAIL_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TempmailApi2TestRunner.getenv(key)
        ...
        explain = TempmailApi2TestRunner.getenv("TEMPMAIL_API2_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if TempmailApi2TestRunner._test_control is not None:
            return TempmailApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TempmailApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TempmailApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TempmailApi2TestRunner.load_test_control()
...
def load_env_local():
    TempmailApi2TestRunner.load_env_local()


def env_override(m):
    return TempmailApi2TestRunner.env_override(m)


def entity_data(v):
    return TempmailApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TempmailApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TempmailApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TempmailApi2TestRunner.load_test_control()


def live_client_options():
    return TempmailApi2TestRunner.live_client_options()


def live_delay_ms():
    return TempmailApi2TestRunner.live_delay_ms()
