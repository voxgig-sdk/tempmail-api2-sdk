...
class TestInboxDirect:
    ...
    def test_should_direct_load_inbox(self):
        setup = _inbox_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-inbox", "live" if setup["live"] else "unit")
        if _skip:
...
def _inbox_direct_setup(mockres):
    ...
    live = env.get("TEMPMAIL_API2_TEST_LIVE") == "TRUE"
