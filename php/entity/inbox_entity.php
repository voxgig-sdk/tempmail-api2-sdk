...
class InboxEntity
{
    ...
    {
        if ($args) {
            $this->_data = TempmailApi2Helpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = TempmailApi2Helpers::to_map(Struct::clone($args)) ?? [];
     ...
     * Load a single Inbox.
     *
     * @param InboxLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed InboxLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Inbox|array The loaded Inbox as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = TempmailApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
