...
class TempmailApi2MakeRequest
{
    public static function call(TempmailApi2Context $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new TempmailApi2Response(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new TempmailApi2Response($fetched);
        } else {
