...
class TempmailApi2SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TempmailApi2Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TempmailApi2Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TempmailApi2Features::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TempmailApi2Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TempmailApi2Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TempmailApi2Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TempmailApi2Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TempmailApi2Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TempmailApi2Error($op . "_allow",
                "TempmailApi2SDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TempmailApi2Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TempmailApi2Error("graphql_error",
                "TempmailApi2SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new TempmailApi2SDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
