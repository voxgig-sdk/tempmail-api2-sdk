...
// TempmailApi2 SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TempmailApi2TestFeature extends TempmailApi2BaseFeature
{
    ...
    public function init(TempmailApi2Context $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TempmailApi2Context $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TempmailApi2Param::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TempmailApi2Context $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TempmailApi2Context $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TempmailApi2Param::call($ctx, $k);
