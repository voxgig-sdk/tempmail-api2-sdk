...
// TempmailApi2 SDK retry feature
...
class TempmailApi2RetryFeature extends TempmailApi2BaseFeature
{
    ...
    public function init(TempmailApi2Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TempmailApi2Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TempmailApi2Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TempmailApi2Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
