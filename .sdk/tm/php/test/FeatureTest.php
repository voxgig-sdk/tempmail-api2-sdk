...
class FtCtrl extends TempmailApi2Control
{
...
class FtClient
{
    ...
    private TempmailApi2Utility $utility;

    public function __construct(TempmailApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): TempmailApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TempmailApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TempmailApi2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility = new TempmailApi2Utility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (TempmailApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TempmailApi2Config::make_feature($name);
    ...
    private static function buildUrl(TempmailApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TempmailApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TempmailApi2Response($fetched);
        }
    ...
    private function fail(TempmailApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TempmailApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
