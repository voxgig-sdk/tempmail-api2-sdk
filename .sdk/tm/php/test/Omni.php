...
//    only - a typed TempmailApi2Context there would make every ctx
...
final class TempmailApi2Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TempmailApi2Context($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new TempmailApi2Spec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new TempmailApi2Result($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new TempmailApi2Error('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new TempmailApi2Response($clean['response']);
    ...
    public static function err_from_map($m): ?TempmailApi2Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TempmailApi2Error(is_string($code) ? $code : '', $msg);
    }
