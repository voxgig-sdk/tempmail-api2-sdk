...
// TempmailApi2 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class TempmailApi2MakePoint
{
    public static function call(TempmailApi2Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof TempmailApi2Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = TempmailApi2Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = TempmailApi2Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
