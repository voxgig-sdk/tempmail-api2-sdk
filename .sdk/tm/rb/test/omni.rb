...
module TempmailApi2Omni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if TempmailApi2Omni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = TempmailApi2Omni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| TempmailApi2Omni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || TempmailApi2Omni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = TempmailApi2Spec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = TempmailApi2Error.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { TempmailApi2Omni.errify(err) }
