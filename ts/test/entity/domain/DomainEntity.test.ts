...
describe('DomainEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set TEMPMAIL_API__TEST_DOMAIN_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TEMPMAIL_API__TEST_DOMAIN_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'TEMPMAIL_API__TEST_DOMAIN_ENTID': idmap,
    'TEMPMAIL_API__TEST_LIVE': 'FALSE',
    'TEMPMAIL_API__TEST_EXPLAIN': 'FALSE',
    'TEMPMAIL_API__APIKEY': 'NONE',
  })

  idmap = env['TEMPMAIL_API__TEST_DOMAIN_ENTID']
