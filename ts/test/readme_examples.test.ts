...
//     (\`new TempmailApi2SDK(...)\` / \`TempmailApi2SDK.test(...)\` become
//     \`TempmailApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TempmailApi2SDK'
...
const TEST_SEED = {"entity":{"domain":{"test01":{"id":"test01"}},"email":{"test01":{"id":"test01"}},"inbox":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TempmailApi2SDK(...)\` and any
// \`TempmailApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TempmailApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TempmailApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
