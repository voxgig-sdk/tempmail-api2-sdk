...
import (
	...
	omni "github.com/voxgig-sdk/tempmail-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TempmailApi2Error); is {
				out := map[string]any{
					"name":    "TempmailApi2Error",
...
func omniCtx(arg any, client *sdk.TempmailApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TempmailApi2SDK); is {
			client = live
