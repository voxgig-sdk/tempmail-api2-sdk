...
func TestDomainEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TEMPMAIL_API2_TEST_DOMAIN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func domainBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TEMPMAIL_API2_TEST_DOMAIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TEMPMAIL_API2_TEST_DOMAIN_ENTID"])
