...
func TestInboxDirect(t *testing.T) {
	t.Run("direct-load-inbox", func(t *testing.T) {
		setup := inboxDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-inbox", _mode); _shouldSkip {
			if _reason == "" {
...
func inboxDirectSetup(mockres any) *inboxDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"TEMPMAIL_API2_TEST_INBOX_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewTempmailApi2SDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["TEMPMAIL_API2_TEST_INBOX_ENTID"]; ok {
